...
export default function Home() {
  ...
  const [mode, setMode] = useState('create'); // Add mode state
  ...
  const handleKnowledgeFileChange = async (e) => {
    ...
    const validFiles = files.filter(file => 
      file.type.match(/(text\/plain|text\/markdown|image\/.*|application\/pdf|application\/msword|application\/vnd.openxmlformats-officedocument.wordprocessingml.document)/) ||
      file.name.endsWith('.md')
    );

    if (validFiles.length !== files.length) {
      alert('Some files were skipped. Only text, markdown, images, PDFs, and Word documents are supported.');
    }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (mode === 'code' || mode === 'text') {
        ...
        knowledgeFiles.forEach((kf, index) => {
          formData.append('knowledgeFiles', kf.file);
        });
      }
          ...
          <Card className="p-6 bg-white/90 backdrop-blur shadow-lg border-green-100
            ...
            {/* Add API key input field */}
                  ...
                  <Button
                    ...
                    disabled={loading || (mode === 'create' && knowledgeFiles.length === 0)}
                  >
                    {loading ? (
                    <>
